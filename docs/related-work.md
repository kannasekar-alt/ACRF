...
### ACRF-01 Implicit Trust Between Agents (AIVSS 9.4)
...
### ACRF-02 No Standard Agent Identity (AIVSS 8.2)
...
### ACRF-03 MCP Server Sprawl (AIVSS 7.2)
...
### ACRF-04 Memory Poisoning (AIVSS 8.6)
...
### ACRF-05 Supply Chain Toxicity (AIVSS 9.2)
...
### ACRF-06 Config Files = Execution Vectors (AIVSS 7.8)
...
### ACRF-07 Multi-Turn Defense Collapse (AIVSS 9.6)
...
### ACRF-08 Cascading Failure Blindness (AIVSS 7.4)
...
### ACRF-09 Semantic Bypass (AIVSS 8.0)
...
### ACRF-10 Safety Controls Not Self-Protecting (AIVSS 9.8)
...
## OWASP AIVSS (AI Vulnerability Severity Scoring System)

The OWASP AIVSS v0.8 scoring system provides a dynamic formula for computing severity scores for agentic AI vulnerabilities: AIVSS = (CVSS_Base + AARS) x Mitigation_Factor, where AARS (Agentic AI Risk Score) is derived from ten risk amplification factors scored per-system. ACRF's AIVSS scores are independently calibrated to the agent-to-agent communication layer rather than computed from the OWASP formula, but the two approaches converge on the same conclusion: agentic systems require severity scoring that goes beyond CVSS. ACRF's per-dimension scores reflect the worst-case blast radius, reversibility, and attacker accessibility specific to A2A communication failures, while OWASP AIVSS captures per-system factors like autonomy level, tool access, and multi-agent interaction. Practitioners using both can cross-reference ACRF's dimension-level scores with system-specific AIVSS computations for a more complete risk picture.
